...
Sortie : probe_traj10_marina_est.json (stations + resume empreinte navire).
...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                   "probe_traj10_marina_est.json")
LMAX = 60.0
NOHIT = 59.0
XS = np.arange(806.0, 850.1, 3.0)
YS = np.arange(-360.0, -239.9, 3.0)
Z_SCOUT = -4.0           # zone peu profonde (plateau -8/-9)
Z_ABOVE_FLOOR = 1.5      # marge reduite (petits fonds)

cfg = {
    "name": "probetraj10", "world": "PierHarbor", "package_name": "Ocean",
    ...
    "agents": [{
        ...
        "control_scheme": 0, "location": [822.0, -330.0, -4.0]}]
}
...
def main():
    ...
    with holoocean.make(scenario_cfg=cfg, show_viewport=False) as env:
        ...
        # ── TEMOINS ──────────────────────────────────────────────────────
        m = mesure(env, agent, 822.0, -330.0, -4.0)
        fond_t = -4.0 - m["LaserDown"]
        ok_f = -9.5 < fond_t < -8.2
        m2 = mesure(env, agent, 819.7, -318.0, -4.0, yaw=180.0)
        ok_q = abs(m2["LaserFwd"] - 5.1) < 1.2
        print(f"TEMOIN fond plateau (822,-330) = {fond_t:.2f} (attendu -8.6..-9.3) -> "
              f"{'OK' if ok_f else 'ECHEC'}")
        print(f"TEMOIN paroi trestle depuis (819.7,-318) cap O fwd = {m2['LaserFwd']:.2f}"
              f" (attendu ~5.1) -> {'OK' if ok_q else 'ECHEC'}")
        res["temoins"] = {"fond": round(fond_t, 2), "trestle_fwd":
            ...
            for y in YS:
                ...
                if m["LaserDown"] >= NOHIT:
                    ...
                    if fond > Z_SCOUT:      # robot sous le fond ? inside-mesh
                        ...
                        fond = None
                if fond is not None and fond + Z_ABOVE_FLOOR < -1.5:
