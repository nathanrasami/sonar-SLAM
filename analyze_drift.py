...
# marqueur Start = départ réel de la GT (plot fidèle, pas de forçage à 0,0)
if gxy_p is not None:
    ax.plot(gxy_p[0, 0], gxy_p[0, 1], marker="*", color="green", markersize=16, label="Start (GT)")

ax.set_xlabel("x (m)")
ax.set_ylabel("y (m)")
ax.set_title("Alignement UMEYAMA (rotation+translation optimales) — plot fidèle à l'ATE")
ax.legend()
ax.axis("equal")
ax.grid(True)
plt.tight_layout()
