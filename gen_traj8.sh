#!/usr/bin/env bash
# Génération traj8 AUTONOME (zone 13 bassin sud, gen_bag_3d_v9.py) : lance,
# surveille, relance si crash, vérifie, notifie. ⚠ v9 REFUSE de tourner sans
# zone13_structures.json (probe E0) au verdict PASS.
# Usage : ./gen_traj8.sh            -> bag complet (~35-60 min, ~11 Go)
#         ./gen_traj8.sh --test 150 -> bag court des checks (~10 min, ~1.3 Go)
#
# - Progression en direct dans le terminal (+ gen_traj8.log).
...
#   tuer et retomber à 0.07 (gen_bag_3d_v9.py OCTREE_MIN_V9).
# - Fin : check_traj4.py --rmax-h 20 --zone zone13 (E1–E9) + notification.
...
LOG="gen_traj8.log"
BAG="BAG_files/holoocean_3d_traj8.bag"
case "$*" in *--test*) BAG="BAG_files/holoocean_3d_traj8_test.bag";; esac

[ -x "$PY" ] || { echo "venv introuvable : $PY"; exit 2; }
[ -f zone13_structures.json ] || { echo "⚠ E0 manquant : lancer probe_traj8_path.py d'abord."; exit 2; }
# garde-fou : un VRAI générateur actif ? (comm=python*, pas les wrappers bash)
for p in $(pgrep -f "gen_bag_3d_v9.py"); do
...
for essai in 1 2 3; do
    ...
    if grep -q "bag ecrit" "$LOG"; then
        echo "=== génération OK, checks E1–E9 (--rmax-h 20 --zone zone13)… ==="
        "$PY" check_traj4.py "$BAG" --rmax-h 20 --zone zone13; rc=$?
        if [ $rc -eq 0 ]; then
            notifier "traj8 : TOUT PASS ✅" "$(tail -1 "$LOG") — E1–E9 PASS ; lancer E10 (conteneur)"
        else
            notifier "traj8 : bag écrit mais E-checks ÉCHEC ⚠" "voir la sortie de check_traj4.py"
        fi
        exit $rc
    fi
    notifier "traj8 : crash moteur (essai $essai/3)" "relance automatique…"
done
notifier "traj8 : ABANDON ✗" "3 crashs consécutifs — voir $LOG"
