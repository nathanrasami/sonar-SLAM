...
therefore validate the 3D extension in the \textbf{HoloOcean} simulator~[11],
...
(table~\ref{tab:holo-sensors}); all measurements are published in the vehicle
...
\item white noise on all channels (gyro, accelerometer, DVL, depth) with the
  standard deviations of table~\ref{tab:holo-sensors}.
\end{itemize}
...
the orthogonal-pair idea of McConnell et al.~[12], we add a second, identical
...
Table~\ref{tab:holo-results} summarises the run. Since the simulator ground
truth is exact and complete, we report the standard Umeyama-aligned
ATE~[7] here (the start-pinned protocol of §~\ref{sec:protocole} was designed
...
The numbers of table~\ref{tab:holo-results} therefore validate the
