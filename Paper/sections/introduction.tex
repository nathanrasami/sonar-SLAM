...
\emph{no} orientation, the initial rotation is set by aligning the \textbf{chord of
the first 15 metres} of track. The ATE is the RMSE of the point-to-point distances
(temporal association). Measured sensitivity to this anchor: a 10/15/20/30/50 m chord
$\to$ ATE 8.6 / 5.6 / 3.6 / 2.3 / 1.9 m on the same run — an initial error of a few
degrees rotates the whole trajectory ($\sim$1.6 m of ATE per degree over a 90 m
footprint). The convention is therefore \emph{fixed} (15 m chord) and identical for
...
section being \textbf{re-anchored at its own start} (translation + local 15 m chord).
...
\item \textbf{Explicit evaluation protocol} (origin anchoring per section, RE, map
  ...
  dataset: relative translation error of \textbf{5.0~\%} against 9.69~\% for the best
  published assisted method (ISOPoT), section-wise anchored ATE shared
