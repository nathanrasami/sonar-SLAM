...
The gain specific to appearance-based detection shows on the metric least sensitive
to anchors: \textbf{Trans. 5.0~\%} (repeatable to 0.01 points between the two runs),
against 4.83--23.6~\% in chapter~1 depending on the seed draw — and above all against
the published methods (§~\ref{sec:compare}). The global anchored ATE (4.8--5.1 m)
remains dominated by the USBL start anchor, as in chapter~1 ($\beta = 90.0$--90.3°
only here: the Sonar Context loops also straighten the heading, where the chapter~1
method let the initial offset drift). The map is at \textbf{0.07 m} (median) from the
true map, p90 0.54 m — the best figure of both chapters
...
The Sonar Context loop closures keep the error within 2--7 m over the 44 min,
against 20 m of accumulated odometry.}
\label{fig:bsu-err}
\end{figure}
...
\caption{Chapter 2 — sections re-anchored at their start. S2 (2.38 m) benefits from
the highest revisit density; S1 pays the noisy start anchor
(§~\ref{sec:protocole}b).}
...
Odom+USBL & \textbf{Adapted Bruce-SLAM (chap.~1)} & 4.79 & 3.55 & \textbf{1.75} & 23.63$^{1}$ & 0.057 \\
Odom+USBL & \textbf{+ Sonar Context (chap.~2)} & 5.13 & \textbf{2.38} & 4.42 & \textbf{5.00} & 0.090 \\
\hline
\end{tabular}
\end{table}
...
\item \textbf{The section-wise anchored ATE is shared}: assisted ISOPoT wins S1
  (3.2 m), we win S2 (2.38 m) and S3 (1.75 m). No method dominates all three
  sections.
...
\item \textbf{Acknowledged protocol difference}: their ``odometry alone'' restarts
  from zero at each section; our methods run for 44 min straight and close loops
  across sections. Finally, none of the published methods delivers a map — we
  deliver trajectory \emph{and} map (0.07 m median, figure~\ref{fig:bsu-cloudgt}).
