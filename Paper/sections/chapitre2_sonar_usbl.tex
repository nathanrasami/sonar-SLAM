...
derived from Sonar Context~\hyperref[bib:sonarcontext]{[2]}, keeping the whole downstream geometric verification
...
Sonar Context~\hyperref[bib:sonarcontext]{[2]} encodes the polar sonar image into an azimuth $\times$ range matrix
...
Each retained candidate follows exactly the Bruce-SLAM pipeline~\hyperref[bib:bruceslam]{[1]}: global shgo
...
ICP with covariance, then pairwise consistency PCM~\hyperref[bib:pcm]{[6]}. Robustness therefore
...
in line with the adaptive USBL factor literature~\hyperref[bib:insusbldvl]{[5]}). In the final configuration
the odometry never reads the USBL at all — not even for its initial heading
(\autoref{sec:protocole}a). We verified this separation directly: the dead-reckoning
...
\item \textbf{Start-pinned ATE of 1.94--2.02 m over the 44 min mission, across three
  ...
  rule (\autoref{sec:protocole}a) and the loops keep it (residual frame offset 0.1°).
...
\item \textbf{The map is at 0.07 m} (median) from the true map, p90 0.51 m — the
  best figure of both chapters (\autoref{fig:bsu-cloudgt}).
\end{itemize}
...
without any orientation re-fit (\autoref{sec:protocole}b).}
...
(\autoref{sec:protocole}e): median 0.07 m, p90 0.51 m.}
...
Same dataset — published rows: Table~I of ISOPoT~\hyperref[bib:isopot]{[4]} (their evaluation toolbox,
sections restarted with the magnetometer heading). Our rows: start-pinned protocol
of \autoref{sec:protocole}a, \emph{stricter} (no orientation re-fit anywhere).
...
\hline
None & SONIC~\hyperref[bib:sonic]{[10]} & 36.6 & 113.3 & 69.8 & 137.65 & 2.45 \\
None & ISOPoT~\hyperref[bib:isopot]{[4]} & 8.8 & 12.7 & 16.7 & 22.76 & 0.99 \\
Odom+Mag & Odometry alone (reset/section) & 5.8 & 12.5 & 6.5 & 19.07 & \textbf{0.0} \\
Odom+Mag & DISO~\hyperref[bib:diso]{[3]} & 5.3 & 6.1 & 10.9 & 13.90 & 0.44 \\
Odom+Mag & SONIC~\hyperref[bib:sonic]{[10]} & 7.0 & 11.2 & 13.7 & 22.83 & \textbf{0.0} \\
Odom+Mag & ISOPoT~\hyperref[bib:isopot]{[4]} & \textbf{3.2} & 3.5 & 4.6 & 9.69 & \textbf{0.0} \\
\hline
...
\noindent Reading, with the precautions of \autoref{sec:protocole}:
...
\item \textbf{The raw frame is part of the result}: the chapter~1 USBL variant
  ...
  corrected (\autoref{sec:chap1-usbl}); with Sonar Context the same convention reads
...
\item \textbf{Protocol check}: our odometry evaluated in a fixed-heading frame
  ...
  Remaining difference, on S3: the exact section boundaries are not published, and
  S3 is the boundary-sensitive one — shifting our S2/S3 boundary by $\pm$3 min
  moves the odometry S3 figure between 6.1 and 9.4 m, and their 6.5 m sits inside
  this envelope. We checked that heading effects do not explain the gap:
  re-anchoring the heading at each section start, or integrating with the
  continuous compass heading, leaves S3 at 9.0--9.2 m. Finally, none of the
  published methods delivers a map — we deliver trajectory \emph{and} map (0.07 m
  median, \autoref{fig:bsu-cloudgt}).
