...
\hline
Pure odometry & — & 19.90--20.10 & — & — & — & 32.5 \\
\hline
...
order of magnitude:
\begin{itemize}
\item from 20 m of raw drift down to \textbf{2.40--3.25 m} of anchored ATE, without
  degrading the heading (median 2.2--3.0°) — figures~\ref{fig:bruce-traj} and
  \ref{fig:bruce-err};
\item per re-anchored section: 2.62 / 4.71 / 2.19 m (run 2,
  figure~\ref{fig:bruce-sections});
...
\item \textbf{$^{1}$The seed artefact on the anchored metric.} The USBL runs show a
  \emph{higher} Trans.~\% and global anchored ATE (4.48--5.60 m) than Bruce alone —
...
\item \textbf{Per section, the anchor pays off where loops become scarce.} With each
  section properly re-anchored, Bruce + USBL wins S3 (\textbf{1.75 m}, best value of
  the chapter) and S2 (3.55 m), where the drift of Bruce alone is no longer corrected
  by revisits. The map metric is unchanged (0.09 m median): the anchor moves the
  trajectory, not the local consistency of the scans.
\end{itemize}
...
start. The global anchored ATE (4.48 m) is dominated by the noisy USBL heading seed
(§~\ref{sec:chap1-usbl}).}
...
\caption{Bruce + USBL — sections re-anchored at their start: S3 = 1.75 m, best value
of the chapter, where the anchor compensates the scarcity of loops.}
