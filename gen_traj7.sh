#!/usr/bin/env bash
# Génération traj7 AUTONOME : lance, surveille, relance si crash, vérifie, notifie.
...
#
# - Progression en direct dans le terminal (+ gen_traj7.log).
...
LOG="gen_traj7.log"
BAG="BAG_files/holoocean_3d_traj7.bag"
case "$*" in *--test*) BAG="BAG_files/holoocean_3d_traj7_test.bag";; esac
...
for p in $(pgrep -f "gen_bag_3d_v7.py"); do
...
for essai in 1 2 3; do
    ...
    if grep -q "bag ecrit" "$LOG"; then
        ...
        if [ $rc -eq 0 ]; then
            notifier "traj7 : TOUT PASS ✅" "$(tail -1 "$LOG") — E1–E9 PASS"
        else
            notifier "traj7 : bag écrit mais E-checks ÉCHEC ⚠" "voir la sortie de check_traj4.py"
        fi
        exit $rc
    fi
    notifier "traj7 : crash moteur (essai $essai/3)" "relance automatique…"
done
notifier "traj7 : ABANDON ✗" "3 crashs consécutifs — voir $LOG"
