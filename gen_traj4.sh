#!/usr/bin/env bash
# Génération traj4 AUTONOME : lance, surveille, relance si crash, vérifie, notifie.
# Usage : ./gen_traj4.sh            -> bag complet (~30 min, 10.3 Go)
#         ./gen_traj4.sh --test 150 -> bag court des checks (~8 min, 1.2 Go)
#
# - Progression en direct dans le terminal (+ gen_traj4.log).
# - Crash moteur (Xid 13 : bag figé, python suspendu) : détecté en ~3 min,
#   python tué, relance automatique (3 essais max).
# - Fin : check_traj4.py (E1–E7) + notification bureau + bip.
set -u
cd "$(dirname "$0")"
PY="$(cd .. && pwd)/holoocean-venv/bin/python"
LOG="gen_traj4.log"
BAG="BAG_files/holoocean_3d_traj4.bag"
case "$*" in *--test*) BAG="BAG_files/holoocean_3d_traj4_test.bag";; esac
...
for p in $(pgrep -f "gen_bag_3d_v4.py"); do
...
for essai in 1 2 3; do
    ...
    if grep -q "bag ecrit" "$LOG"; then
        echo "=== génération OK, checks E1–E7… ==="
        "$PY" check_traj4.py "$BAG"; rc=$?
        if [ $rc -eq 0 ]; then
            notifier "traj4 : TOUT PASS ✅" "$(tail -1 "$LOG") — E1–E7 PASS"
        else
            notifier "traj4 : bag écrit mais E-checks ÉCHEC ⚠" "voir la sortie de check_traj4.py"
        fi
        exit $rc
    fi
    notifier "traj4 : crash moteur (essai $essai/3)" "relance automatique…"
done
notifier "traj4 : ABANDON ✗" "3 crashs consécutifs — voir $LOG"
